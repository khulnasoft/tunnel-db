...
import (
	...
	gh "go.khulnasoft.com/tunnel-db/pkg/github"
)
...
func TestClient_UploadReleaseAssets(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			files: map[string][]byte{
				"tunnel.db.gz":       []byte("full"),
				"tunnel-light.db.gz": []byte("light"),
			},
			filePaths: []string{
				"tunnel.db.gz",
				"tunnel-light.db.gz",
			...
			uploadReleaseAsset: []gh.UploadReleaseAssetExpectation{
				{
					Args: gh.UploadReleaseAssetArgs{
						Ctx:          context.Background(),
						Id:           1,
						Opt:          &github.UploadOptions{Name: "tunnel.db.gz", Label: "", MediaType: "application/gzip"},
						FileAnything: true,
					},
					Returns: gh.UploadReleaseAssetReturns{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			dir, err := ioutil.TempDir("", "tunnel-db")
